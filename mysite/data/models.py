...
class Answer(models.Model):
	...
	def save(self, *args, **kwargs):
		...
		if results.count() == 0:
			newFeedObject = FeedObject(product1=self.chosenProduct, image1=self.chosenProduct.fileURL, fbFriend1=self.fromUser, product1Count = 1,  product2=self.wrongProduct, image2=self.wrongProduct.fileURL, fbFriend2=None, product2Count = 0, currentQuestion=self.question, questionText = self.question.text, forUser= self.forFacebookId)    
			newFeedObject.save()
		else:
			newFeedObject = results[0]
			#determines ordering of the products
			if self.chosenProduct == newFeedObject.product1:
				newFeedObject.fbFriend1 = self.fromUser
				newFeedObject.product1Count = newFeedObject.product1Count + 1
				
			else:
				newFeedObject.fbFriend2 = self.fromUser
